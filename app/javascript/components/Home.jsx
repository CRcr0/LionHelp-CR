...
class Home extends React.Component{
    ...
    componentDidMount(){
        if(localStorage.getItem('token')!=null){
            ...
            const params = "/"+this.state.search+"/"+this.state.location+"/"+this.state.sort;
            fetch(url)
                .then(response => {
                if (response.ok) {
    ...
    handleSubmit(event){
        ...
        if(localStorage.getItem('token')!=null){
            ...
            // if(this.state.location!='NA') localStorage.setItem("location", this.state.location);
            // if(this.state.sort!='NA') localStorage.setItem("sort", this.state.sort);
            // fetch(url+params)
            // .then(response => {
            // if (response.ok) {
            //     return response.json();
    ...
    handleChangeSelected(postID){
        this.state.current_post_id = postID
    ...
    commentContent() {
        ...
        if (this.props.login) {
            ...
            const url = `/api/v1/posts/comment?post_id=${this.state.current_post_id}&content=${$(".comment-input").val()}&commenter=${this.props.email}`;
            fetch(url)
                .then(response => {
    ...
    toggleComment(){
        ...
        if (this.props.login) {
            // console.log("=========================")
            // console.log(this.props.email)
            const url = `/api/v1/posts/comment_list?post_id=${this.state.current_post_id}`;
            fetch(url)
                .then(response => {
                if (response.ok) {
                    return response.json();
                ...
                .then(response => {
                    this.setState({ selectedComments: response })
                }
                    )
                .catch(() => this.props.history.push("/"));
        } else {
            ...
            <>
                ...
                <Modal isOpen={this.state.isCommentOpen} toggle={this.toggleComment}>
                    ...
                    <ModalBody style={{marginTop:"20px"}}>
                        <FormGroup>
                            <Input  type="text" className="comment-input" name="search" placeholder="Please input your comment" onChange={this.handleInputChange}/>
                        </FormGroup>
                        <Button type="submit" className="searchButton" style={{backgroundColor:"white", border:"none"}} onClick={this.commentContent}>
                            <b style={{color:"rgb(2, 33, 105)"}}><center>Submit</center></b>
                        </Button>
                        {comment}
                    </ModalBody>
                </Modal>
